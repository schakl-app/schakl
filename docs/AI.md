...
to draw. The review slide-over that opens beside the message whenever the fill-in was asked
for — after the approve, and after a `.eml` upload or a Gmail import filed onto a task
(`TaskReviewDialog`, docs/UX.md) — is the other reader of that column: it hands the strip its
own `reveal`, re-reads the row when the run lands and adopts only the fields the reader has not
touched. A
