...
129 endpoints across 4 groups. A permission written `module.resource.action:own` is scoped: `:own` covers rows that are yours, `:any` covers everyone's, and `:any` satisfies a check for `:own`.
...
Returns: `200` · `AiVisibilityImportResult`
...
Returns: `200` · `GtmWorkspaceContentsRead`
