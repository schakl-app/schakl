...
112 endpoints across 4 groups. A permission written `module.resource.action:own` is scoped: `:own` covers rows that are yours, `:any` covers everyone's, and `:any` satisfies a check for `:own`.
...
| Name | Type | Meaning |
| --- | --- | --- |
| `source` | `subscription` · `domain` · required | subscription \| domain |
| `source_id` | string · required | the agreement's or domain's id |

Returns: `200` · `BilledPeriod[]`
...
Returns: `200` · `OriginalsBatchReport`
...
Returns: `200` · `InvoicingRevenueStats`
...
Path parameters: `subscription_id`, `entity_type`, `entity_id`
...
Permission: `subscriptions.subscription.read:any`
...
Returns: `200` · `SubscriptionTypeSaved`
