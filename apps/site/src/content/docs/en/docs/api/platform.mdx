...
279 endpoints across 20 groups. A permission written `module.resource.action:own` is scoped: `:own` covers rows that are yours, `:any` covers everyone's, and `:any` satisfies a check for `:own`.
...
| `GET` `/custom-fields/scopes` | `settings.customfields.read` |
...
#### `GET /api/v1/custom-fields/scopes`

**List Scopes** — The dimensions a definition of ``entity_type`` may be narrowed to, with this tenant's options for each (``scoping.py``). Empty for an entity type no module has registered a dimension for — which is what tells the settings screen not to draw the control (#253).
...
Returns: `200` · `CustomFieldScopeRead[]`
...
| Endpoint | Permission |
| --- | --- |
| `GET` `/google-search-console/ai-visibility` | `google_search_console.site.read` |
| `GET` `/google-search-console/breakdown` | `google_search_console.site.read` |
| `GET` `/google-search-console/hourly` | `google_search_console.site.read` |
| `GET` `/google-search-console/inspect` | `google_search_console.site.read` |
| `GET` `/google-search-console/movers` | `google_search_console.site.read` |
| `GET` `/google-search-console/overview` | `google_search_console.site.read` |
| `GET` `/google-search-console/query` | `google_search_console.report.run` |
| `GET` `/google-search-console/search-types` | `google_search_console.site.read` |
| `GET` `/google-search-console/site` | `google_search_console.site.read` |
| `GET` `/google-search-console/sitemap` | `google_search_console.site.read` |
| `GET` `/google-search-console/sitemaps` | `google_search_console.site.read` |
| `GET` `/google-search-console/sites` | `google_search_console.site.read` |
| `GET` `/google-search-console/timeseries` | `google_search_console.site.read` |

<details>
<summary>Parameters and schemas per endpoint</summary>

#### `GET /api/v1/google-search-console/ai-visibility`

**Google Search Console Ai Visibility** — How visible the site is in Google's generative AI features — AI Overviews and AI Mode — as far as the Search Console API can say.

Permission: `google_search_console.site.read`

Query parameters:

| Name | Type | Meaning |
| --- | --- | --- |
| `site` | string · required | The property exactly as GET /sites lists it: sc-domain:klant.nl for a domain property or https://www.klant.nl/ (trailing slash included) for a URL-prefix property. A bare hostname is read as the domain property. |
| `period` | string | The span to report on: a trailing window (30d, 90d, 365d), a preset (month, last_month, quarter, last_quarter) or a named calendar period (2026-07, 2026-Q3). An unknown value falls back to 30d. Search Console keeps its days in Pacific time; the last two or three days are still being collected and are flagged as fresh. |
| `compare` | string | year (default — what seasonality survives) or previous. |

Returns: `200` · `GoogleSearchConsoleAiVisibility`

#### `GET /api/v1/google-search-console/breakdown`

**Google Search Console Breakdown** — One dimension, ranked — top queries, top pages, countries, devices, or the search appearances (rich results, video, product snippets…) the site was shown with.
...
| --- | --- | --- |
| `site` | string · required | The property exactly as GET /sites lists it: sc-domain:klant.nl for a domain property or https://www.klant.nl/ (trailing slash included) for a URL-prefix property. A bare hostname is read as the domain property. |
| `dimension` | string | query (default), page, country, device or searchAppearance. (default `query`) |
| `period` | string | The span to report on: a trailing window (30d, 90d, 365d), a preset (month, last_month, quarter, last_quarter) or a named calendar period (2026-07, 2026-Q3). An unknown value falls back to 30d. Search Console keeps its days in Pacific time; the last two or three days are still being collected and are flagged as fresh. |
| `search_type` | string | web (default; includes AI Overviews, which cannot be split out), image, video, news, discover or googleNews. |
| `filters` | string[] | Dimension filters, repeatable: dimension==value, dimension!=value, dimension=@value (contains), dimension!@value, dimension=~regex, dimension!~regex, on query, page, country (ISO 3166-1 alpha-3, lower case: nld), device (DESKTOP/MOBILE/TABLET) or searchAppearance. All clauses must match. A clause that parses as none of these is refused rather than ignored. |
| `limit` | integer |  (default `25`) |
| `offset` | integer |  (default `0`) |
| `order` | string | Google ranks by clicks. Another metric (-impressions, -ctr, +position) is applied over the first thousand clicks-ranked rows, and the answer says so in warnings. |

Returns: `200` · `GoogleSearchConsoleReport`

#### `GET /api/v1/google-search-console/hourly`

**Google Search Console Hourly** — Hour by hour over the last few days, today included and still moving — for "what happened this morning" and "did the launch land". Every row may still change; there is no final hourly data, by Google's design.
...
| --- | --- | --- |
| `site` | string · required | The property exactly as GET /sites lists it: sc-domain:klant.nl for a domain property or https://www.klant.nl/ (trailing slash included) for a URL-prefix property. A bare hostname is read as the domain property. |
| `days` | integer | How many days back, today included. Google keeps ten days of hourly rows. (default `2`) |
| `search_type` | string | web (default; includes AI Overviews, which cannot be split out), image, video, news, discover or googleNews. |

Returns: `200` · `GoogleSearchConsoleReport`

#### `GET /api/v1/google-search-console/inspect`

**Google Search Console Inspect Url** — What Google's index holds for one URL: whether it is indexed (verdict PASS) and if not why (coverage_state, indexing_state, robots_txt_state, page_fetch_state), which canonical Google chose against the one the page declares, when it was last crawled, and the rich results found. One URL per call — Google allows 2 000 inspections a day per property.
...
| --- | --- | --- |
| `site` | string · required | The property exactly as GET /sites lists it: sc-domain:klant.nl for a domain property or https://www.klant.nl/ (trailing slash included) for a URL-prefix property. A bare hostname is read as the domain property. |
| `url` | string · required | The page to inspect. Must be under the property. |
| `language` | string | BCP-47 code for the issue texts, e.g. nl. |

Returns: `200` · `GoogleSearchConsoleInspection`

#### `GET /api/v1/google-search-console/movers`

**Google Search Console Movers** — Which queries or pages moved most in average position between this period and the one it is compared against. change is positive for a climb. entered and dropped count what appeared or vanished entirely, which the rows cannot show.
...
| `period` | string | The span to report on: a trailing window (30d, 90d, 365d), a preset (month, last_month, quarter, last_quarter) or a named calendar period (2026-07, 2026-Q3). An unknown value falls back to 30d. Search Console keeps its days in Pacific time; the last two or three days are still being collected and are flagged as fresh. |
| `compare` | string | year (default — what seasonality survives) or previous. |
| `dimension` | string | query (default) or page. (default `query`) |
| `limit` | integer |  (default `25`) |
| `min_impressions` | number | Ignore anything shown fewer times than this in either span. (default `10`) |
| `search_type` | string | web (default; includes AI Overviews, which cannot be split out), image, video, news, discover or googleNews. |

Returns: `200` · `GoogleSearchConsoleMovers`

#### `GET /api/v1/google-search-console/overview`

**Google Search Console Overview** — How a property did over a period — clicks, impressions, CTR and average position — against the same period a year earlier, with the change already computed, plus the split by device.
...
Returns: `200` · `GoogleSearchConsoleOverview`

#### `GET /api/v1/google-search-console/query`

**Google Search Console Query** — Any dimensions crossed, with any filters, any aggregation and any data state — the escape hatch for questions the curated reads do not answer (query by page, page by country, branded queries by device…).

Permission: `google_search_console.report.run`
...
| --- | --- | --- |
| `site` | string · required | The property exactly as GET /sites lists it: sc-domain:klant.nl for a domain property or https://www.klant.nl/ (trailing slash included) for a URL-prefix property. A bare hostname is read as the domain property. |
| `dimensions` | string[] | Repeatable, in row-key order: query, page, country, device, searchAppearance, date, hour. None is a total. hour forces data_state hourly_all. |
| `period` | string | The span to report on: a trailing window (30d, 90d, 365d), a preset (month, last_month, quarter, last_quarter) or a named calendar period (2026-07, 2026-Q3). An unknown value falls back to 30d. Search Console keeps its days in Pacific time; the last two or three days are still being collected and are flagged as fresh. |
| `search_type` | string | web (default; includes AI Overviews, which cannot be split out), image, video, news, discover or googleNews. |
| `filters` | string[] | Dimension filters, repeatable: dimension==value, dimension!=value, dimension=@value (contains), dimension!@value, dimension=~regex, dimension!~regex, on query, page, country (ISO 3166-1 alpha-3, lower case: nld), device (DESKTOP/MOBILE/TABLET) or searchAppearance. All clauses must match. A clause that parses as none of these is refused rather than ignored. |
| `aggregation` | string | auto (default), byProperty, byPage or byNewsShowcasePanel. |
| `data_state` | string | all (default here: includes the fresh, still-changing days), final (Google's default: finalised data only, so the last days are missing) or hourly_all. |
| `limit` | integer |  (default `25`) |
| `offset` | integer |  (default `0`) |

Returns: `200` · `GoogleSearchConsoleReport`

#### `GET /api/v1/google-search-console/search-types`

**Google Search Console Search Types** — The four metrics per Google surface — web, image, video, news, Discover, Google News — so "where is this site actually seen" is one answer. AI Overviews sit inside web and the API cannot split them out; see /ai-visibility for what it can say.
...
Returns: `200` · `GoogleSearchConsoleSearchTypes`

#### `GET /api/v1/google-search-console/site`

**Get Google Search Console Site** — One property's record: its kind (domain or URL prefix) and what this account may do there.
...
Returns: `200` · `GoogleSearchConsoleSite`

#### `GET /api/v1/google-search-console/sitemap`

**Google Search Console Sitemap** — One sitemap by its URL.
...
| --- | --- | --- |
| `site` | string · required | The property exactly as GET /sites lists it: sc-domain:klant.nl for a domain property or https://www.klant.nl/ (trailing slash included) for a URL-prefix property. A bare hostname is read as the domain property. |
| `feedpath` | string · required | The sitemap's own URL, as /sitemaps lists it. |

Returns: `200` · `GoogleSearchConsoleSitemap`

#### `GET /api/v1/google-search-console/sitemaps`

**Google Search Console Sitemaps** — The sitemaps submitted for a property: when Google last read each, how many URLs it declares, and how many errors and warnings it carries. A sitemap nobody submitted, or one with errors, is the usual answer to "why are the new pages not indexed".
...
Returns: `200` · `GoogleSearchConsoleSitemapList`

#### `GET /api/v1/google-search-console/sites`

**List Google Search Console Sites** — Every Search Console property the signed-in user's Google account can read. Start here.
...
| Name | Type | Meaning |
| --- | --- | --- |
| `query` | string | Filter by site URL. |

Returns: `200` · `GoogleSearchConsoleSiteList`

#### `GET /api/v1/google-search-console/timeseries`

**Google Search Console Timeseries** — The four metrics day by day, oldest first. A day with nothing shown is omitted by Google rather than answered as zero.
...
| `GET` `/impex/invoice/columns` | `invoicing.invoice.read` |
| `GET` `/impex/invoice/export` | `impex.export`, `invoicing.invoice.read` |
| `POST` `/impex/invoice/import` | `impex.import`, `invoicing.invoice.write` |
| `POST` `/impex/invoice/inspect` | `impex.import`, `invoicing.invoice.write` |
...
#### `GET /api/v1/impex/invoice/columns`

**Impex Columns Invoice** — Every column a invoice import can write into: the entity's own, those contributed by other modules, and this organisation's custom fields — with the labels, types and aliases a mapping UI needs.

Permission: `invoicing.invoice.read`

Returns: `200` · `ImpexColumnsResponse`

#### `GET /api/v1/impex/invoice/export`

**Impex Export Invoice** — Export the current filtered invoice list as CSV (UTF-8, BOM). Headers are stable column keys plus the tenant's custom-field keys — the file re-imports into the same organisation unchanged.

Permission: `impex.export`, `invoicing.invoice.read`
...
| `status` | string | Status, as on the list |
| `company_id` | string |  |
| `overdue` | boolean | Only rows past their due date (default `false`) |
| `sort` | string | List sort key, '-' desc |

Returns: `200`

#### `POST /api/v1/impex/invoice/import`

**Impex Import Invoice** — Import invoice rows from a spreadsheet, upserting on the first of `number` each row fills (max 2000 data rows per request). Accepts a CSV/TSV/Excel upload or a pasted block; the format is read from the content, not the filename.

Permission: `impex.import`, `invoicing.invoice.write`
...
Request body: `Body_impex_import_invoice_api_v1_impex_invoice_import_post`

Returns: `200` · `ImportReport`

#### `POST /api/v1/impex/invoice/inspect`

**Impex Inspect Invoice** — Read an uploaded file and report what it is — format, worksheets, encoding, row count — plus each of its columns with sample cells and the suggested target column. Writes nothing and reads no records; returns a fingerprint the import repeats so a mapping cannot be applied to a different file.

Permission: `impex.import`, `invoicing.invoice.write`

Request body: `Body_impex_inspect_invoice_api_v1_impex_invoice_inspect_post`
...
| Endpoint | Permission |
| --- | --- |
| `GET` `/meetings` | `meetings.meeting.read` |
| `POST` `/meetings` | `meetings.meeting.write` |
| `GET` `/meetings/{meeting_id}` | `meetings.meeting.read` |
| `PATCH` `/meetings/{meeting_id}` | `meetings.meeting.write` |
| `DELETE` `/meetings/{meeting_id}` | `meetings.meeting.delete` |
| `POST` `/meetings/{meeting_id}/ai/revise` | `meetings.meeting.write` |
| `POST` `/meetings/{meeting_id}/ai/transcribe` | `meetings.meeting.write` |
| `DELETE` `/meetings/{meeting_id}/audio` | `meetings.meeting.write` |
| `POST` `/meetings/{meeting_id}/chunks` | `meetings.meeting.write` |
| `POST` `/meetings/{meeting_id}/confirm` | `meetings.meeting.write` |
| `POST` `/meetings/{meeting_id}/finish` | `meetings.meeting.write` |
| `PUT` `/meetings/{meeting_id}/minutes` | `meetings.meeting.write` |
| `PUT` `/meetings/{meeting_id}/participants` | `meetings.meeting.write` |
| `GET` `/meetings/{meeting_id}/pdf` | `meetings.meeting.read` |
| `GET` `/meetings/{meeting_id}/preview` | `meetings.meeting.read` |
| `POST` `/meetings/{meeting_id}/redraft` | `meetings.meeting.write` |
| `POST` `/meetings/{meeting_id}/retry` | `meetings.meeting.write` |
| `GET` `/meetings/{meeting_id}/status` | `meetings.meeting.read` |
| `GET` `/meetings/{meeting_id}/transcript` | `meetings.meeting.read` |
| `GET` `/meetings/policy` | `meetings.meeting.write` |
| `GET` `/meetings/settings` | `meetings.settings.manage` |
| `PUT` `/meetings/settings` | `meetings.settings.manage` |
| `GET` `/meetings/settings/designs/{design}/source` | `meetings.settings.manage` |
| `POST` `/meetings/settings/preview` | `meetings.settings.manage` |
| `GET` `/meetings/settings/sections` | `meetings.settings.manage` |
...
Permission: `meetings.meeting.read`
...
| `company_id` | string |  |
| `project_id` | string |  |
| `status` | string | Comma-separated set; absent means every status |
| `q` | string |  |
| `limit` | integer |  (default `50`) |
| `offset` | integer |  (default `0`) |
| `count` | boolean |  (default `true`) |

Returns: `200` · `MeetingList`

#### `POST /api/v1/meetings`

**Create Meeting** — Open a recording. Refused unless the caller states the participants were told.

Permission: `meetings.meeting.write`
...
Path parameters: `meeting_id`

Returns: `200` · `MeetingDetail`
...
Permission: `meetings.meeting.delete`

Path parameters: `meeting_id`

Returns: `204`

#### `POST /api/v1/meetings/{meeting_id}/ai/revise`

**Revise Meeting With Ai** — One typed instruction, applied to the meeting as the caller: the title, client, project, kind and date, the roster and its speaker labels, and — while under review — every part of the minutes. The answer is a diff; what the instruction did not mention stays.

Permission: `meetings.meeting.write`

Path parameters: `meeting_id`

Request body: `MeetingReviseRequest`

Returns: `200` · `MeetingReviseResult`

#### `POST /api/v1/meetings/{meeting_id}/ai/transcribe`

**Transcribe Revise Instruction** — Speech to text for the revise box: an instruction spoken instead of typed. Nothing is written; the words come back to be read first.

Permission: `meetings.meeting.write`

Path parameters: `meeting_id`

Request body: `MeetingTranscribeRequest`

Returns: `200` · `TimeTranscribeResult`

#### `DELETE /api/v1/meetings/{meeting_id}/audio`

**Delete Audio** — Drop the recording now rather than at the retention date; the words stay.
...
#### `POST /api/v1/meetings/{meeting_id}/chunks`

**Add Chunk** — One piece of the recording, base64 in JSON, while it is still being recorded.

Permission: `meetings.meeting.write`

Path parameters: `meeting_id`

Request body: `MeetingChunk`

Returns: `200` · `object`

#### `POST /api/v1/meetings/{meeting_id}/confirm`

**Confirm Meeting** — The minutes become a contact moment and the ticked action items become tasks.

Permission: `meetings.meeting.write`

Path parameters: `meeting_id`

Request body: `MeetingConfirm`

Returns: `200` · `MeetingConfirmResult`

#### `POST /api/v1/meetings/{meeting_id}/finish`

**Finish Meeting** — The recorder stopped: the worker folds, transcribes and drafts.

Permission: `meetings.meeting.write`

Path parameters: `meeting_id`

Request body: `MeetingFinish`

Returns: `200` · `MeetingDetail`

#### `PUT /api/v1/meetings/{meeting_id}/minutes`

**Save Minutes** — The reviewer's edits to the draft, kept without confirming.

Permission: `meetings.meeting.write`

Path parameters: `meeting_id`

Request body: `MinutesDraft`

Returns: `200` · `MeetingDetail`

#### `PUT /api/v1/meetings/{meeting_id}/participants`

**Set Participants** — Who was in the meeting — a colleague, a contact of the client, or a name — and which speaker label (S1, S2 …) each of them is. Replaces the whole roster.

Permission: `meetings.meeting.write`

Path parameters: `meeting_id`

Request body: `MeetingParticipants`

Returns: `200` · `MeetingDetail`

#### `GET /api/v1/meetings/{meeting_id}/pdf`

**Meeting Pdf** — The minutes as a PDF, with the chapters the caller ticked (the transcript is off unless asked for). A read, so it survives an expired licence.
...
| Name | Type | Meaning |
| --- | --- | --- |
| `sections` | string | Comma-separated chapters; absent = the org's defaults |

Returns: `200`

#### `GET /api/v1/meetings/{meeting_id}/preview`

**Preview Meeting** — The minutes document as HTML — the same artefact the PDF prints.
...
#### `POST /api/v1/meetings/{meeting_id}/redraft`

**Redraft Meeting** — Draft the minutes again over the transcript already here — after naming the speakers, so the action items land on the people who took them on. No new transcription.
...
**Meeting Status** — The one column the detail page polls while a worker holds the row.

Permission: `meetings.meeting.read`

Path parameters: `meeting_id`

Returns: `200` · `MeetingStatusRead`

#### `GET /api/v1/meetings/{meeting_id}/transcript`

**Meeting Transcript** — The transcript, whole: as JSON (every line with its speaker's name and the seconds it was said at — what an agent reads), or as a `.txt`, `.md`, `.srt` or `.vtt` file.
...
#### `GET /api/v1/meetings/policy`

**Meeting Policy** — What the recorder asks before it records: readable by whoever may record.

Permission: `meetings.meeting.write`

Returns: `200` · `MeetingPolicy`

#### `GET /api/v1/meetings/settings`

**Get Settings** — The org's meetings settings: the consent statement, the minutes document, the house writing instructions. No saved row means the defaults.

Permission: `meetings.settings.manage`

Returns: `200` · `MeetingSettingsRead`

#### `PUT /api/v1/meetings/settings`

**Update Settings**

Permission: `meetings.settings.manage`

Request body: `MeetingSettingsUpdate`

Returns: `200` · `MeetingSettingsRead`

#### `GET /api/v1/meetings/settings/designs/{design}/source`

**Design Source** — A shipped design's own HTML and CSS, to start a custom minutes template from.

Permission: `meetings.settings.manage`

Path parameters: `design`

Returns: `200` · `MeetingDesignSource`

#### `POST /api/v1/meetings/settings/preview`

**Preview Settings** — Render an unsaved design over the org's latest minuted meeting — the editor's preview.

Permission: `meetings.settings.manage`

Request body: `MeetingSettingsPreviewRequest`

Returns: `200`

#### `GET /api/v1/meetings/settings/sections`

**Section Catalog** — Every chapter a minutes document can carry, with the org's default for each.

Permission: `meetings.settings.manage`

Returns: `200` · `MeetingSectionCatalogEntry[]`
...
| Endpoint | Permission |
| --- | --- |
| `GET` `/microsoft/calendar/calendars` | `microsoft.connection.manage` |
| `PUT` `/microsoft/calendar/calendars` | `microsoft.connection.manage` |
| `GET` `/microsoft/calendar/channels` | `microsoft.calendar.read` |
| `GET` `/microsoft/calendar/events` | `microsoft.calendar.read` |
| `POST` `/microsoft/calendar/webhook` | _Deliberately open_ |
| `GET` `/microsoft/connections` | `microsoft.settings.manage` |
| `GET` `/microsoft/connections/me` | `microsoft.connection.manage` |
| `PATCH` `/microsoft/connections/me` | `microsoft.connection.manage` |
| `POST` `/microsoft/connections/me/disconnect` | `microsoft.connection.manage` |
| `GET` `/microsoft/oauth/callback` | `microsoft.connection.manage` |
| `GET` `/microsoft/oauth/connect` | `microsoft.connection.manage` |
| `GET` `/microsoft/onedrive/browse` | `microsoft.onedrive.read` |
| `DELETE` `/microsoft/onedrive/files/{drive_id}/{item_id}` | `microsoft.onedrive.write` |
| `PUT` `/microsoft/onedrive/folder` | `microsoft.onedrive.write` |
| `POST` `/microsoft/onedrive/folders` | `microsoft.onedrive.write` |
| `GET` `/microsoft/onedrive/links` | `microsoft.onedrive.read` |
| `POST` `/microsoft/onedrive/links` | `microsoft.onedrive.write` |
| `DELETE` `/microsoft/onedrive/links/{link_id}` | `microsoft.onedrive.write` |
| `POST` `/microsoft/onedrive/provision` | `microsoft.onedrive.write` |
| `POST` `/microsoft/onedrive/provision-all` | `microsoft.settings.manage` |
| `GET` `/microsoft/onedrive/state` | `microsoft.onedrive.read` |
| `POST` `/microsoft/onedrive/upload-session` | `microsoft.onedrive.write` |
| `POST` `/microsoft/outlook/import` | `interactions.interaction.write` |
| `GET` `/microsoft/outlook/lookup` | `microsoft.connection.manage` |
| `POST` `/microsoft/outlook/refresh` | `microsoft.connection.manage` |
| `GET` `/microsoft/outlook/search` | `microsoft.connection.manage` |
| `GET` `/microsoft/outlook/status` | `microsoft.connection.manage` |
| `GET` `/microsoft/outlook/threads/{conversation_id}` | `microsoft.connection.manage` |
| `GET` `/microsoft/settings` | `microsoft.settings.manage` |
| `PUT` `/microsoft/settings` | `microsoft.settings.manage` |
...
**Microsoft Calendar List** — The viewer's own calendars — the selection UI's read. Briefly cached; the existing ``Calendars.ReadWrite`` scope covers it, so no re-consent is needed.

Permission: `microsoft.connection.manage`

Returns: `200` · `MicrosoftCalendarListEntry[]`

#### `PUT /api/v1/microsoft/calendar/calendars`

**Microsoft Set Calendar Selection** — Choose which extra calendars sync for the viewer. Deselecting removes the calendar's cached events on the spot; selecting queues a sync so the agenda fills without waiting.

Permission: `microsoft.connection.manage`

Request body: `MicrosoftCalendarSelection`

Returns: `200` · `MicrosoftCalendarListEntry[]`

#### `GET /api/v1/microsoft/calendar/channels`

**Microsoft Calendar Channels** — The viewer's selection off the database alone — what the Agenda's feeds menu reads on every open, so it never costs a Graph call.

Permission: `microsoft.calendar.read`

Returns: `200` · `MicrosoftSelectedCalendar[]`

#### `GET /api/v1/microsoft/calendar/events`

**Microsoft Calendar Events** — The viewer's own cached Outlook events. Reads the local cache, never Graph live.
...
Returns: `200` · `MicrosoftCalendarFeedItem[]`
...
Permission: `microsoft.settings.manage`

Returns: `200` · `MicrosoftConnectionRead[]`

#### `GET /api/v1/microsoft/connections/me`

**My Connection**

Permission: `microsoft.connection.manage`

Returns: `200` · `MyMicrosoftConnectionRead`

#### `PATCH /api/v1/microsoft/connections/me`

**Update My Connection**

Permission: `microsoft.connection.manage`

Request body: `MyMicrosoftConnectionUpdate`

Returns: `200` · `MyMicrosoftConnectionRead`

#### `POST /api/v1/microsoft/connections/me/disconnect`
...
**Microsoft Oauth Callback** — Store the grant and land the browser back where the connect started.

Permission: `microsoft.connection.manage`

Returns: `200`

#### `GET /api/v1/microsoft/oauth/connect`

**Oauth Connect** — 302 to Microsoft's consent screen, asking exactly the enabled surfaces' scopes.
...
| Name | Type | Meaning |
| --- | --- | --- |
| `include_outlook` | boolean |  (default `false`) |
| `next` | string |  (default ``) |

Returns: `200`

#### `GET /api/v1/microsoft/onedrive/browse`

**Browse** — Live folder contents **as the viewing user** — Graph's permissions are authoritative.

Permission: `microsoft.onedrive.read`
...
Returns: `200` · `OneDriveBrowseResult`

#### `DELETE /api/v1/microsoft/onedrive/files/{drive_id}/{item_id}`

**Trash Item** — Move an item to the drive's recycle bin — the other half of unlink. Runs as the viewing user; drops every link naming the item, org-wide; refuses a non-empty folder.

Permission: `microsoft.onedrive.write`

Path parameters: `drive_id`, `item_id`

Returns: `204`

#### `PUT /api/v1/microsoft/onedrive/folder`

**Set Folder** — Point a record at an existing folder — the picker's target. The service adds ``microsoft.onedrive.manage`` when the record **already has** a folder (§15's two layers).

Permission: `microsoft.onedrive.write`

Request body: `OneDriveFolderSet`

Returns: `200` · `OneDriveLinkRead`

#### `POST /api/v1/microsoft/onedrive/folders`

**Create Folder** — Create a subfolder inside the folder being browsed, as the viewing user.

Permission: `microsoft.onedrive.write`

Request body: `OneDriveFolderCreate`

Returns: `201` · `OneDriveFolder`

#### `GET /api/v1/microsoft/onedrive/links`
...
#### `POST /api/v1/microsoft/onedrive/links`

**Create Link**

Permission: `microsoft.onedrive.write`

Request body: `OneDriveLinkCreate`

Returns: `201` · `OneDriveLinkRead`

#### `DELETE /api/v1/microsoft/onedrive/links/{link_id}`

**Delete Link** — Unlink only. The item is never touched (the dialog says so too).
...
#### `POST /api/v1/microsoft/onedrive/provision`

**Provision Entity** — Queue one entity's folder — the panel's "create folder" button.

Permission: `microsoft.onedrive.write`

Request body: `OneDriveProvisionRequest`

Returns: `202`

#### `POST /api/v1/microsoft/onedrive/provision-all`

**Provision All** — Backfill: a folder for every client that has none (Instellingen → Microsoft 365).

Permission: `microsoft.settings.manage`

Returns: `200` · `OneDriveBulkProvisionResult`

#### `GET /api/v1/microsoft/onedrive/state`
...
Returns: `200` · `OneDriveStateRead`

#### `POST /api/v1/microsoft/onedrive/upload-session`

**Create Upload Session**

Permission: `microsoft.onedrive.write`

Request body: `OneDriveUploadSessionCreate`

Returns: `200` · `OneDriveUploadSession`

#### `POST /api/v1/microsoft/outlook/import`

**Import Outlook Message** — Log one message the poller skipped, filed where the caller says. The declared key is the one for the row this **writes**; reaching into the mailbox is asked for in the service.

Permission: `interactions.interaction.write`

Request body: `OutlookImportRequest`

Returns: `201` · `OutlookImportResult`

#### `GET /api/v1/microsoft/outlook/lookup`

**Lookup Outlook Message** — Resolve a pasted reference to the conversation it names, in the caller's own mailbox.
...
| Name | Type | Meaning |
| --- | --- | --- |
| `reference` | string · required | An Outlook on the web link, a Graph message id, or an RFC-822 Message-ID |

Returns: `200` · `OutlookLookupResult`

#### `POST /api/v1/microsoft/outlook/refresh`

**Refresh Outlook** — Poll this mailbox once, now — rate-limited to one manual poll per minute.

Permission: `microsoft.connection.manage`

Returns: `200` · `OutlookRefreshResult`

#### `GET /api/v1/microsoft/outlook/search`

**Search Outlook** — Find a message in the caller's **own** mailbox, by who it was with and when — named fields, never raw KQL, so a colon in an address cannot become an operator.
...
Returns: `200` · `OutlookSearchResult`

#### `GET /api/v1/microsoft/outlook/status`

**Read Outlook Status** — When this mailbox was last polled, and whether asking for another one is worth it.

Permission: `microsoft.connection.manage`

Returns: `200` · `OutlookSyncStatus`

#### `GET /api/v1/microsoft/outlook/threads/{conversation_id}`

**Read Outlook Thread** — Every message of one conversation, marked with what is already on the timeline.
...
#### `GET /api/v1/microsoft/settings`

**Get Settings**

Permission: `microsoft.settings.manage`

Returns: `200` · `MicrosoftSettingsRead`

#### `PUT /api/v1/microsoft/settings`

**Save Settings**

Permission: `microsoft.settings.manage`

Request body: `MicrosoftSettingsWrite`
...
| Endpoint | Permission |
| --- | --- |
| `GET` `/trash/company` | `companies.company.delete` |
| `GET` `/trash/company/{entity_id}` | `companies.company.delete` |
| `DELETE` `/trash/company/{entity_id}` | `companies.company.delete` |
| `GET` `/trash/company/{entity_id}/preview` | `companies.company.delete` |
| `POST` `/trash/company/{entity_id}/restore` | `companies.company.delete` |

<details>
<summary>Parameters and schemas per endpoint</summary>

#### `GET /api/v1/trash/company`

**Trash List Company** — The company records in the trash: who deleted each, when, when it will be purged, and what hangs off it. A record with anything in `blocking` is kept past the retention window until somebody restores it.
...
#### `GET /api/v1/trash/company/{entity_id}`

**Trash Get Company** — One trashed company record. 404 for a live record: to this surface a row that is not in the trash does not exist.

Permission: `companies.company.delete`

Path parameters: `entity_id`

Returns: `200` · `TrashItem`

#### `DELETE /api/v1/trash/company/{entity_id}`

**Trash Purge Company** — Delete a trashed company record for good. Irreversible; refused (409) while anything in `blocking` still hangs off it. The nightly sweep does this by itself after the retention window.
...
#### `GET /api/v1/trash/company/{entity_id}/preview`

**Trash Preview Company** — What deleting a live company record would do: `blocking` lists the records that stop it (issued invoices, domains, agreements, projects, hours — these outlive a client, so a client holding any is archived rather than deleted), `taken_along` what hides with it and goes when it is purged. Read this before calling DELETE.

Permission: `companies.company.delete`

Path parameters: `entity_id`

Returns: `200` · `TrashPreview`

#### `POST /api/v1/trash/company/{entity_id}/restore`

**Trash Restore Company** — Bring a trashed company record back exactly as it was, with everything that hid with it.
