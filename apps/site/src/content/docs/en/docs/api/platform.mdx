...
267 endpoints across 20 groups. A permission written `module.resource.action:own` is scoped: `:own` covers rows that are yours, `:any` covers everyone's, and `:any` satisfies a check for `:own`.
...
| `PUT` `/meetings/{meeting_id}/speakers` | `meetings.meeting.write` |
...
#### `PUT /api/v1/meetings/{meeting_id}/speakers`

**Set Speakers** — Name the provider's speaker labels — "S2 is Jan".

Permission: `meetings.meeting.write`

Path parameters: `meeting_id`

Request body: `MeetingSpeakers`
