...
112 endpoints, verdeeld over 4 groepen. Een recht geschreven als `module.resource.action:own` heeft een bereik: `:own` dekt rijen die van jou zijn, `:any` die van iedereen, en `:any` voldoet aan een controle op `:own`.

## `invoicing`

| Endpoint | Recht |
| --- | --- |
| `GET` `/invoicing/billed-periods` | `invoicing.invoice.read` |
...
| `GET` `/invoicing/stats/revenue` | `invoicing.invoice.read:any` |
...
#### `GET /api/v1/invoicing/billed-periods`

**Billed Periods** — Which periods of one agreement or domain a document already holds, and on which document — the record's own invoicing history, newest first.
...
| Naam | Type | Betekenis |
| --- | --- | --- |
| `source` | `subscription` · `domain` · verplicht | subscription \| domain |
| `source_id` | string · verplicht | the agreement's or domain's id |

Antwoord: `200` · `BilledPeriod[]`
...
| Naam | Type | Betekenis |
| --- | --- | --- |
| `force` | boolean | Delete an issued invoice as well. Off, only a draft deletes and an issued one answers 409 (cancel it, or credit it). On, the number leaves the sequence for good; refused while payments, credit notes, a ledger booking or an open online checkout hang off it. (default `false`) |
...
**Credit Invoice** — Credit note mirroring this invoice with negated prices — a draft to edit down, or with ``issue=true`` a definitive one that writes the invoice off at once and hands its work back (the way to cancel an invoice the client has already received).

Recht: `invoicing.invoice.write`

Padparameters: `invoice_id`

Request body: `InvoiceCredit`
...
#### `GET /api/v1/invoicing/stats/revenue`

**Invoicing Revenue Stats** — What was invoiced in ``year`` beside the year before: per month, per client, per kind, excl. and incl. tax. The ledger's turnover — ``time/stats/revenue`` is the hours' worth.
...
Antwoord: `200` · `InvoicingRevenueStats`
...
| `POST` `/subscriptions/{subscription_id}/links` | `subscriptions.subscription.write` |
| `DELETE` `/subscriptions/{subscription_id}/links/{entity_type}/{entity_id}` | `subscriptions.subscription.write` |
...
| `GET` `/subscriptions/summary` | `subscriptions.subscription.read:any` |
| `GET` `/subscriptions/templates` | `subscriptions.subscription.read:any` |
...
| `linkable` | boolean | with entity_type/entity_id: the agreements that could be attached to the record instead of the ones already on it — its client's, alive, of a kind that attaches to it, not yet linked (default `false`) |
...
#### `POST /api/v1/subscriptions/{subscription_id}/links`

**Link Subscription** — Attach one project, task or website to the agreement. A website needs a kind that ``covers_websites``; a link already there is left as it is.

Recht: `subscriptions.subscription.write`

Padparameters: `subscription_id`

Request body: `SubscriptionLinkWrite`

Antwoord: `201` · `SubscriptionRead`

#### `DELETE /api/v1/subscriptions/{subscription_id}/links/{entity_type}/{entity_id}`

**Unlink Subscription** — Detach one record from the agreement.

Recht: `subscriptions.subscription.write`

Padparameters: `subscription_id`, `entity_type`, `entity_id`
...
Recht: `subscriptions.subscription.read:any`
...
**Update Subscription Type** — Flipping ``billed_in_advance`` re-reads the periods of every agreement of this kind; ``shifted_subscriptions`` reports how many, so the screen can say so.
...
Antwoord: `200` · `SubscriptionTypeSaved`
