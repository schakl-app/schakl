...
113 endpoints, verdeeld over 4 groepen. Een recht geschreven als `module.resource.action:own` heeft een bereik: `:own` dekt rijen die van jou zijn, `:any` die van iedereen, en `:any` voldoet aan een controle op `:own`.
...
| `burn` | string | 'over' (at or past budget), 'warn' (75-100 %) or 'ok' (under 75 %) keeps only that burn band; other tokens ignored |
...
| `POST` `/tasks/{task_id}/ai/checklist` | `tasks.task.write` |
| `POST` `/tasks/{task_id}/ai/revise` | `tasks.task.write` |
| `POST` `/tasks/{task_id}/ai/transcribe` | `tasks.task.write` |
...
| `GET` `/tasks/intake` | `tasks.task.create` |
| `DELETE` `/tasks/intake/{intake_id}` | `tasks.task.create` |
| `POST` `/tasks/intake/{intake_id}/create` | `tasks.task.create` |
| `GET` `/tasks/intake/summary` | `tasks.task.create` |
...
| `GET` `/tasks/settings` | `tasks.settings.manage` |
| `PUT` `/tasks/settings` | `tasks.settings.manage` |
...
| `assigned_to` | `contact` · `agency` | Whose work it is: `contact` — assigned to one of the client's own people; `agency` — everything else (assigned to staff, or to nobody yet). The client's homepage draws the two as separate tiles, and a specific person is a different question (`assignee_contact_id` / `assignee_user_id`). |
...
| `series_id` | string | Every task of one schedule-mode series — the root that holds the repeat rule and each occurrence laid out from it, finished or not. Any member's id names the series; a task that is not in one answers an empty page. |
| `collapse_series` | boolean | Fold each schedule-mode series onto its current occurrence: an unfinished occurrence due after today that is not the series' earliest unfinished one is left out, and the row that stands for them carries `series_pending`. Off by default — the export and the MCP surface read the whole list. (default `false`) |
...
#### `POST /api/v1/tasks/{task_id}/ai/checklist`

**Generate Checklist With Ai** — Write this task's steps from its title and notes, as one new checklist.

Recht: `tasks.task.write`

Padparameters: `task_id`

Request body: `TaskChecklistGenerateRequest`

Antwoord: `201` · `ChecklistRead`

#### `POST /api/v1/tasks/{task_id}/ai/revise`

**Revise Task With Ai** — Change this task in words: one typed instruction, applied as the caller.

Recht: `tasks.task.write`

Padparameters: `task_id`

Request body: `TaskReviseRequest`

Antwoord: `200` · `TaskReviseResult`

#### `POST /api/v1/tasks/{task_id}/ai/transcribe`

**Transcribe Revise Instruction** — Speech to text for the revise box: an instruction spoken instead of typed.
...
#### `GET /api/v1/tasks/intake`

**List Intake** — The caller's own mails to the task address — parked ones first in the UI, recent ones for the record. Never another sender's: a mail is its sender's until it is a task.
...
Antwoord: `200` · `TaskIntakeRead[]`

#### `DELETE /api/v1/tasks/intake/{intake_id}`

**Discard Intake** — Throw a parked mail away. The receipt stays, so the same mail cannot come back.

Recht: `tasks.task.create`

Padparameters: `intake_id`

Antwoord: `204`

#### `POST /api/v1/tasks/intake/{intake_id}/create`

**Complete Intake** — Finish a parked mail by hand: name the client, and the task is created as the caller.

Recht: `tasks.task.create`

Padparameters: `intake_id`

Request body: `TaskIntakeComplete`

Antwoord: `200` · `TaskRead`

#### `GET /api/v1/tasks/intake/summary`

**Intake Summary** — How many of the caller's mails wait for a client — the strip on the board.

Recht: `tasks.task.create`

Antwoord: `200` · `TaskIntakeSummary`
...
**List Schedules** — Planned blocks in a window: the caller's own, named people's (``user_ids``, ``:any`` for anyone else), or everybody's (``all_users``, ``:any``; a holder of ``:own`` gets their own).
...
| Naam | Type | Betekenis |
| --- | --- | --- |
| `user_ids` | string[] · verplicht |  |
| `date_from` | string · verplicht |  |
| `date_to` | string · verplicht |  |

Antwoord: `200` · `BusyFeedRead`

#### `GET /api/v1/tasks/settings`

**Get Settings** — The org's tasks settings: the e-mail intake address. No saved row means the defaults.

Recht: `tasks.settings.manage`

Antwoord: `200` · `TaskSettingsRead`
...
Request body: `TaskSettingsUpdate`
...
| `GET` `/time/stats/projects` | `time.report.read` |
...
#### `GET /api/v1/time/stats/projects`

**Project Time Stats** — Per-project minutes (all / billable / approved / invoiced) and the billable worth — what the projects report joins onto each budget. Unbounded dates mean all time.
