...
INVOICING_TRASH_DEPENDENTS = (
    ...
    # A one-time sale nobody invoiced is a note about the client and goes with them; one a
    # document bills is protected by that document's own rule above.
    TrashDependent(
        key="invoicing.sales_open",
        label_key="trash.dependent.invoicing.sales_open",
        blocks=False,
        count=count_by_column(
            "invoicing_product_sales", "company_id", where="AND invoice_id IS NULL"
        ),
    ),
)
