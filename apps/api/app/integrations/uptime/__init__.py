...
from app.integrations.uptime import status as _status  # noqa: F401
from app.integrations.uptime.automation import UPTIME_AUTOMATION_ACTIONS
from app.integrations.uptime.bulk import UPTIME_MONITOR_BULK
from app.integrations.uptime.impex import UPTIME_MONITOR_IMPEX, UPTIME_WEBSITE_COLUMNS
from app.integrations.uptime.jobs import prune_heartbeats
from app.integrations.uptime.panels import UPTIME_PANELS
from app.integrations.uptime.permissions import UPTIME_PERMISSIONS
from app.integrations.uptime.router import router
...
module = ModuleDescriptor(
    ...
    # Requires nothing, and that is a decision rather than an oversight. Its panels attach to
    # **both** `websites` and `domains`, so the honest requirement is "websites *or* domains" —
    # which an AND-list cannot say. Declaring `websites` would make a domains-only tenant switch
    # on a module they do not use to watch a host they do, and over-declaring is the failure
    # direction this field must not have (CLAUDE.md §6a).
    requires=(),
    router=router,
