...
from app.integrations.cloudflare.models import CloudflareAccount, CloudflareRegistrarDomain
from app.integrations.cloudflare.permissions import CLOUDFLARE_PERMISSIONS
from app.integrations.cloudflare.router import router
...
module = ModuleDescriptor(
    ...
    # Requires `domains`: every surface this contributes hangs off a domain — the zone panel is
    # an `entityType: "domain"` panel, and the redirect rule exists to put a mechanism behind
    # `Domain.status = redirect`. With `domains` off it is a settings screen holding a credential
