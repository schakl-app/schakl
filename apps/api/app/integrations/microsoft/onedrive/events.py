"""Event handlers: a new client (or project) gets its folder in the agency's library.

In-transaction and write-only, like every bus handler: the folder job row commits with the
company/project itself; the worker does the Graph I/O with the org's automation connection.
"""
...
from app.integrations.microsoft.onedrive.service import drive_root, queue_folder_job
...
async def _provisioning_on(ctx: EmitContext) -> bool:
    ...
    return bool(
        row is not None
        and row.onedrive_enabled
        and row.onedrive_auto_provision
