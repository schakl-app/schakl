...
declines has a name (:mod:`~app.integrations.google.gmail.gates`). The manual importer's explainer
...
from app.integrations.google.gmail.gates import (
    PERSISTED_REASONS,
    Decision,
    GateCache,
    SkipReason,
...
from app.integrations.google.oauth import SCOPE_GMAIL, google_settings_row
...
async def message_already_here(
    ...
    *and* the chain stays one function. Duplicating them here would be exactly the drift
    :mod:`~app.integrations.google.gmail.gates` exists to prevent, one gate earlier.
    """
...
async def gmail_mailboxes(session: AsyncSession, org_id: uuid.UUID) -> list[Mailbox]:
    """This provider's mailboxes for the shared *who polls what* composition
    (:mod:`app.core.mailbox.internals`): the address each grant was made with, and whether that
    mailbox genuinely polls — active, opted in, holding the Gmail scope. Same predicate the cron
    offers on (``jobs.google_gmail_poll``); stated once there and once here is already one copy
    too many, but the alternative is the cron importing this module to ask."""
    rows = await session.execute(
        select(
            GoogleConnection.user_id,
        ...
        .order_by(GoogleConnection.created_at)
    )
    return [
        Mailbox(
            email=email.lower(),
...
#: Who counts as *us* — the shared composition, read here under the name the rest of the
#: module and the manual importer have always used.
...
_match_contacts = match_contacts
