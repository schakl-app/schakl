...
**Explaining is a dry run, not a log** (:mod:`~app.integrations.google.gmail.gates`). Every reason
...
from app.integrations.google.client import acting_as, is_oauth_error, mark_connection_error
from app.integrations.google.gmail import matching
from app.integrations.google.gmail.gates import GateCache
from app.integrations.google.gmail.models import GmailSuppression
...
class GmailCandidate(BaseModel):
    ...
    #: :func:`~app.integrations.google.gmail.service.classify` the ingest runs, asked about this one
...
async def _explain(
    ...
    **The same decision the poller makes, asked rather than performed.** Every reason here
    comes from :func:`~app.integrations.google.gmail.service.classify`; nothing about the gates is
