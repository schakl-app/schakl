...
async def entity_activity(
    ...
    if not is_auditable(entity_type):
        return []
    # A portal login reads dashboards, not the staff paper trail. That check used to live here,
    # and here only — which is why the company hub's core panel, the *other* reader of the same
    # service, served a client fifteen lines of it. It is `ActivityService.feed`'s now, so this
    # route inherits it and so does everything written next; a client still gets an empty feed
    # rather than a 403, because they hold the read permissions these panels ask for.
    #
