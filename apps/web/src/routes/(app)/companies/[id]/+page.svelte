...
<div class="mb-6">
    ...
    <div>
      ...
      {#if company.website}
        <a
          ...
          class="mt-1 inline-block text-sm text-neutral-500 hover:text-brand"
        >{company.website} ↗</a>
      {/if}
    </div>
    <div class="flex flex-wrap items-center gap-2">
      <a href={`/tasks?company_id=${company.id}`}
      ...
      <button class="rounded-lg bg-brand px-3 py-1.5 text-sm font-medium text-white hover:opacity-90"
        onclick={() => (showEdit = true)}>
        {t("common.edit")}
      </button>
      <button class="rounded-lg border border-neutral-300 px-3 py-1.5 text-sm text-neutral-400 hover:border-red-300 hover:text-red-600"
  ...
  {#if data.templates.length > 0}
    <form method="POST" action="?/applyTemplate" use:enhance class="mt-3 flex items-center gap-2">
      <select name="template_id" class="rounded-lg border border-neutral-300 px-2 py-1.5 text-sm" required>
        {#each data.templates as template (template.id)}
          <option value={template.id}>{template.name}</option>
        {/each}
      </select>
      <button class="rounded-lg border border-neutral-300 px-3 py-1.5 text-sm text-neutral-600 hover:border-brand hover:text-brand">
...
<Modal bind:open={showEdit} title={t("companies.edit")}>
  <form method="POST" action="?/update"
    use:enhance={() => ({ update }) => { showEdit = false; void update(); }}
    class="space-y-3">
    <div>
      <label for="edit-name" class="mb-1 block text-sm font-medium text-neutral-700">{t("companies.name")}</label>
      <input id="edit-name" name="name" value={company.name} required class={inputClass} />
    </div>
    <div class="grid grid-cols-2 gap-3">
      <div>
        <label for="edit-website" class="mb-1 block text-sm font-medium text-neutral-700">{t("companies.website")}</label>
        <input id="edit-website" name="website" value={company.website ?? ""} class={inputClass} />
      </div>
      <div>
        <label for="edit-status" class="mb-1 block text-sm font-medium text-neutral-700">{t("companies.field.status")}</label>
        <select id="edit-status" name="status" class={inputClass}>
          {#each COMPANY_STATUSES as status (status)}
            <option value={status} selected={company.status === status}>{t(`companies.status.${status}`)}</option>
          {/each}
        </select>
      </div>
    </div>
    <div>
      <label for="edit-notes" class="mb-1 block text-sm font-medium text-neutral-700">{t("companies.notes")}</label>
      <textarea id="edit-notes" name="notes" rows="3" class={inputClass}>{company.notes ?? ""}</textarea>
    </div>
    {#if data.definitions.length > 0}
      <CustomFieldsForm definitions={data.definitions} values={company.custom ?? {}} locale={data.locale} />
    {:else}
    ...
    <div class="flex justify-end gap-2 pt-1">
      <button type="button" class="rounded-lg border border-neutral-300 px-4 py-2 text-sm" onclick={() => (showEdit = false)}>
        {t("common.cancel")}
      </button>
