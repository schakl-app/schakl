...
{#if showInvite}
  ...
  >
    <div class="grid gap-3 sm:grid-cols-3">
      <div>
        <label for="email" class="mb-1 block text-sm font-medium text-neutral-700">{t("settings.users.email")}</label>
        <input id="email" name="email" type="email" required class={inputClass} />
      </div>
      <div>
        <label for="full_name" class="mb-1 block text-sm font-medium text-neutral-700">{t("settings.users.name")}</label>
        <input id="full_name" name="full_name" class={inputClass} />
      </div>
      <div>
        <label for="role" class="mb-1 block text-sm font-medium text-neutral-700">{t("settings.users.role")}</label>
        <select id="role" name="role" class={inputClass}>
          {#each data.roles as r (r)}<option value={r} selected={r === "member"}>{t(`roles.${r}`)}</option>{/each}
        </select>
      </div>
    </div>
...
{#if data.members.length === 0}
  <p class="rounded-xl border border-dashed border-neutral-300 bg-white p-8 text-center text-sm text-neutral-500">
      ...
      <li class="flex items-center gap-3 px-4 py-3">
        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-2">
            <span class="truncate font-medium text-neutral-900">{member.full_name || member.email}</span>
            {#if member.is_self}
              <span class="rounded-full bg-brand/10 px-2 py-0.5 text-[11px] font-medium text-brand">{t("settings.users.you")}</span>
            {/if}
            {#if !member.is_active}
              <span class="rounded-full bg-amber-100 px-2 py-0.5 text-[11px] font-medium text-amber-700">{t("settings.users.inactive")}</span>
            {/if}
          </div>
        ...
        <form method="POST" action="?/changeRole" use:enhance>
          ...
          >
            {#each data.roles as r (r)}<option value={r} selected={member.role === r}>{t(`roles.${r}`)}</option>{/each}
          </select>
        </form>
        ...
        {#if !member.is_self}
          <form
            ...
            use:enhance
            onsubmit={(e) => {
              if (!confirm(t("settings.users.revoke_confirm"))) e.preventDefault();
            }}
          >
            <input type="hidden" name="membership_id" value={member.membership_id} />
            <button class="text-sm text-neutral-400 hover:text-red-600">{t("settings.users.revoke")}</button>
          </form>
        {/if}
      </li>
    {/each}
