...
<div class="max-w-2xl space-y-6">
  ...
  <!-- Personal API keys moved to their own screen (Instellingen → API en MCP): the flow grew a
       target picker, scope presets and a connection guide, none of which is about the person. -->
  {#if data.canManageKeys}
    <section class="rounded-xl border border-border bg-surface-raised p-5">
      <h2 class="text-sm font-semibold text-text">{t("settings.api.title")}</h2>
      <p class="mt-1 text-sm text-text-muted">{t("settings.account.api_moved")}</p>
      <a
        href="/settings/api"
        class="mt-3 inline-flex items-center rounded-lg border border-border px-3 py-1.5 text-sm text-text hover:border-brand"
      >
        {t("settings.account.api_open")}
      </a>
    </section>
  {/if}
