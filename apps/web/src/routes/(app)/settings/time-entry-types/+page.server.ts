...
export const load: PageServerLoad = async (event) => {
  if (!can(event.locals.user, "time.entry_type.manage")) throw redirect(303, "/settings");
  const { data } = await apiFor(event).GET("/api/v1/time/entry-types", {
...
export const actions: Actions = {
  save: async (event) => {
    ...
    if (type_id) {
      const { error } = await apiFor(event).PATCH("/api/v1/time/entry-types/{type_id}", {
        params: { path: { type_id } },
      ...
      const { error, response } = await apiFor(event).POST("/api/v1/time/entry-types", {
  ...
  toggle: async (event) => {
    ...
    const type_id = String(form.get("id") ?? "");
    if (type_id) {
      await apiFor(event).PATCH("/api/v1/time/entry-types/{type_id}", {
        params: { path: { type_id } },
  ...
  delete: async (event) => {
    ...
    const type_id = String(form.get("id") ?? "");
    if (type_id) {
      const { error } = await apiFor(event).DELETE("/api/v1/time/entry-types/{type_id}", {
        params: { path: { type_id } },
      });
      if (error) return fail(400, { error: apiErrorKey(error).key });
    }
