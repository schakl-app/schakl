...
<div class="mb-6 flex items-center justify-between">
  <div>
    <h1 class="text-xl font-semibold text-neutral-900">{t("contacts.title")}</h1>
    <p class="mt-1 text-sm text-neutral-500">{t("contacts.count", { count: data.total })}</p>
  </div>
...
{#if data.contacts.length === 0}
  <div class="rounded-xl border border-dashed border-neutral-300 bg-white p-10 text-center">
    <p class="font-medium text-neutral-900">{t("contacts.empty")}</p>
