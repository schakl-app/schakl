...
export const load: PageServerLoad = async (event) => {
  if (!can(event.locals.user, "invoicing.quote.write")) throw redirect(303, "/quotes");
...
export const actions: Actions = {
  create: async (event) => {
    ...
    const { data, error } = await apiFor(event).POST("/api/v1/invoicing/quotes", {
      body: {
        ...body,
        company_id,
    ...
    throw redirect(303, `/quotes/${data.id}`);
  },
