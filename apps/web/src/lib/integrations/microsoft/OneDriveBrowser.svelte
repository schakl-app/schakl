<script lang="ts">
  /**
   * The embedded OneDrive folder browser: breadcrumb, folders-first list, open in OneDrive,
   * upload, and link-to-record — our own browse-and-pick, no OneDrive file picker (that would
   * drag in a browser SDK and a second consent; we already have an authenticated API).
   *
   * The same deliberate exceptions to the SSR-only data path that the Drive browser makes:
   * - The **listing** loads from the browser after mount. A live Graph round trip inside a
   ...
   * - **Upload bytes go straight to Microsoft**: the API only mints the upload session; the
   *   browser PUTs the file to the pre-authenticated `uploadUrl` in fragments, each with a
   *   `Content-Range`. File contents never transit our API. Graph wants fragments in multiples
   *   of 320 KiB and caps one at 60 MiB; 10 MiB (32 × 320 KiB) is well inside both.
   * - **Creating a folder** posts to `/api/v1/microsoft/onedrive/folders` from here (same
   *   viewer-scoped API, same-origin cookie), then re-lists — it needs no host action and no
   *   page data.
   * - **An upload links itself to the record it was uploaded from** (#328's rule), by the same
   *   same-origin POST the ＋ on a row makes. The listing is live and the link list is what
   *   survives the page; "it landed in this record's own folder" is not the same fact as "this
   *   record has this file", and only the second one is still true tomorrow.
   *
   * A Graph item is addressed by **two** ids — the drive and the item — so every crumb, every
   * row and every form here carries the pair; a folder id alone names nothing.
   *
   * In **pick mode** (`pick`) the same browser chooses a *folder* instead of linking files:
   * every folder row offers "choose", and so does the folder you are standing in.
   *
   * **Searching is the API's job.** The listing is one page of 100 items, so a filter box over
   * `listing.items` would answer "geen resultaten" for a file that is merely 101st
   * alphabetically. `q` goes to Graph; unlike Drive, Graph's search covers the folder *and its
   * subfolders*, and the header sentence says so rather than presenting a subtree as a folder.
   *
   * **A file's name links the file.** You are in this browser because you came to attach
   * something; opening it in OneDrive is the incidental act and the row already carries ↗ for
   * it. Where there is no link action to take (`pick` mode, or a caller who cannot write) the
   * name falls back to opening OneDrive: a control that always refuses is worse than none.
   *
   * **A file can be removed from where it was uploaded** (#394's rule). Every row carries the
   * same ⋯ item the panel's link list does: *Verwijderen uit OneDrive*, which moves the item to
   * the recycle bin and drops every link naming it. A refusal — Graph's, or "deze map is niet
   * leeg" — is shown as its own strip **above the list rather than instead of it**.
   *
   * **Host contract:** the page exposes `?/linkOneDriveFile`, and `?/setOneDriveFolder` when
   * the host renders this in pick mode (spread `oneDriveActions`).
   */
  import {
    Check,
    ChevronLeft,
  ...
  interface Listing {
    ...
    query: string | null;
    /** Graph had a page two nobody follows: this list is a prefix of the folder. */
    truncated: boolean;
  }
  ...
  let {
    ...
    /** The drive browsing starts in; null with `rootFolderId` null = the org's root. */
    rootDriveId?: string | null;
    /** Where browsing starts; null = the org's client-folders parent. */
    rootFolderId?: string | null;
    /** The record rows get linked to via the host's `?/linkOneDriveFile` action. */
    entityType: string;
    entityId: string;
    canWrite?: boolean;
    /** Choose a folder for the record (`?/setOneDriveFolder`) instead of linking files. */
    pick?: boolean;
  ...
  //: Graph accepts a fragment of at most 60 MiB and wants multiples of 320 KiB; 32 of those.
  const FRAGMENT_BYTES = 32 * 320 * 1024;

  // Only a *successful* pick closes the picker: a refused one (a member without
  // `microsoft.onedrive.manage` re-pointing a folder) must leave the browser where it stands.
  const picked =
    () =>
  ...
  // svelte-ignore state_referenced_locally
  let crumbs = $state<Crumb[]>([{ driveId: rootDriveId, id: rootFolderId, name: null }]);
  let listing = $state<Listing | null>(null);
  let loading = $state(false);
  let errorKey = $state("");
  let uploading = $state(false);
  // "New folder" affordance: an inline name field, opened from the header.
  ...
  const searchSummary = $derived.by(() => {
    ...
    const params = {
      ...
      term: listing.query,
      folder: listing.folder.name ?? t("microsoft.onedrive.root"),
    };
    if (listing.items.length === 0) return t("microsoft.onedrive.search_none", params);
    return tn("microsoft.onedrive.search_results", listing.items.length, params);
  });
  ...
  async function load(refresh = false) {
    ...
    if (current.driveId) params.set("drive_id", current.driveId);
    ...
    try {
      const response = await fetch(`/api/v1/microsoft/onedrive/browse?${params}`, {
        headers: { accept: "application/json" },
      });
      if (!response.ok) {
        const body = await response.json().catch(() => null);
        errorKey = body?.error?.message ?? "errors.microsoft_onedrive_unavailable";
        listing = null;
        return;
      }
      listing = (await response.json()) as Listing;
      // Name the crumb from the listing that *is* it. The root is the one crumb created without
      // a name — it cannot have one before the first read (#363). A name is written once and
      // never re-read; the drive id the root resolved to is written with it, so the org root
      // (asked for as "no ids") is addressable by the pair from then on.
      const name = listing.folder.name;
      if (crumbs[target] && crumbs[target].id === targetId && !crumbs[target].name) {
        crumbs = crumbs.map((crumb, index) =>
          index === target
            ? {
                driveId: crumb.driveId ?? listing?.folder.drive_id ?? null,
                id: crumb.id ?? listing?.folder.id ?? null,
                name: name ?? crumb.name,
              }
            : crumb,
        );
      }
    } catch {
      errorKey = "errors.microsoft_onedrive_unavailable";
      listing = null;
    } finally {
      loading = false;
    }
  ...
  function open(item: BrowseItem) {
    ...
    crumbs = [...crumbs, { driveId: item.drive_id, id: item.id, name: item.name }];
    clearSearch(false);
    void load();
  }
  ...
  /** Debounced so typing costs one Graph round-trip, not one per keystroke. */
  ...
  /**
   * A crumb's label. Only the crumb we are *standing on* may borrow the listing's name — that
   * is the one case where "we don't know it yet" and "it is the folder on screen" are the same
   * sentence. An ancestor borrowing it renders somewhere else's name (#363).
   */
  function crumbLabel(crumb: Crumb, index: number): string {
    if (crumb.name) return crumb.name;
    if (index === crumbs.length - 1 && listing?.folder?.name) return listing.folder.name;
    return t("microsoft.onedrive.root");
  }
  ...
  async function createFolder() {
    ...
    const driveId = listing?.folder?.drive_id;
    if (!name || !parentId || !driveId || savingFolder) return;
    savingFolder = true;
    errorKey = "";
    try {
      const response = await fetch("/api/v1/microsoft/onedrive/folders", {
        method: "POST",
        headers: { "content-type": "application/json", accept: "application/json" },
        body: JSON.stringify({ drive_id: driveId, parent_id: parentId, name }),
      });
      if (!response.ok) {
        const body = await response.json().catch(() => null);
        errorKey = body?.error?.message ?? "errors.microsoft_onedrive_folder_failed";
        return;
      }
      newFolderName = "";
      ...
      errorKey = "errors.microsoft_onedrive_folder_failed";
    } finally {
      savingFolder = false;
    }
  }
  ...
  /**
   * Attach a just-uploaded item to the record this browser hangs off. The upload itself has
   * already succeeded by the time this runs, so a refusal here reports *that* — telling
   * someone their upload failed when the file is sitting in OneDrive sends them to look for a
   * problem that isn't there.
   */
  async function linkUploaded(driveId: string, itemId: string) {
    try {
      const response = await fetch("/api/v1/microsoft/onedrive/links", {
        method: "POST",
        headers: { "content-type": "application/json", accept: "application/json" },
        body: JSON.stringify({
      ...
      if (!response.ok) {
        const body = await response.json().catch(() => null);
        errorKey = body?.error?.message ?? "errors.microsoft_onedrive_link_failed";
        return;
      }
      ...
      errorKey = "errors.microsoft_onedrive_link_failed";
    }
  }

  /**
   * Bin the item itself. Graph's permissions answer (the API acts as this user), so a refusal
   * here is Microsoft's own and is reported as such; a non-empty folder is refused by the API
  ...
  async function trash() {
    ...
    try {
      const response = await fetch(
        `/api/v1/microsoft/onedrive/files/${encodeURIComponent(item.drive_id)}/${encodeURIComponent(item.id)}`,
        { method: "DELETE", headers: { accept: "application/json" } },
      );
      if (!response.ok) {
        const body = await response.json().catch(() => null);
        actionErrorKey = body?.error?.message ?? "errors.microsoft_onedrive_unavailable";
        return;
      }
      ...
      actionErrorKey = "errors.microsoft_onedrive_unavailable";
    } finally {
      trashing = false;
      confirmTrash = false;
      trashTarget = null;
  ...
  /**
   * The bytes go browser → Microsoft in `Content-Range` fragments against the session URL the
   * API minted. Sequential on purpose: Graph requires fragments in order, and the last
   * fragment's response is the driveItem the record then gets linked to.
   */
  async function putFragments(sessionUri: string, file: File): Promise<{ id?: string } | null> {
    let offset = 0;
    let last: Response | null = null;
    while (offset < file.size) {
      const end = Math.min(offset + FRAGMENT_BYTES, file.size);
      last = await fetch(sessionUri, {
        method: "PUT",
    ...
    if (!last) return null;
    return (await last.json().catch(() => null)) as { id?: string } | null;
  }
  ...
  async function upload(input: HTMLInputElement) {
    ...
    const driveId = listing?.folder?.drive_id;
    if (!file || !folderId || !driveId) return;
    uploading = true;
    errorKey = "";
    try {
      if (file.size === 0) {
        // An upload session with nothing to put in it never completes; say so up front.
        errorKey = "errors.microsoft_upload_failed";
        return;
      }
      const session = await fetch("/api/v1/microsoft/onedrive/upload-session", {
        method: "POST",
        headers: { "content-type": "application/json", accept: "application/json" },
        body: JSON.stringify({
          drive_id: driveId,
      ...
      if (!session.ok) {
        ...
        // account" and "your account was connected before OneDrive was switched on" surface,
        // and both are actionable where the generic upload failure is not.
        const body = await session.json().catch(() => null);
        errorKey = body?.error?.message ?? "errors.microsoft_upload_failed";
        return;
      }
      const { session_uri } = (await session.json()) as { session_uri: string };
      const uploaded = await putFragments(session_uri, file);
      if (uploaded === null) {
        errorKey = "errors.microsoft_upload_failed";
        return;
      }
      // The completed session answers with the driveItem; its id is what the record gets
      // attached to. A body we cannot read is not an upload failure — the bytes are in
      // OneDrive and the listing below will show them.
      if (uploaded.id) await linkUploaded(driveId, uploaded.id);
      await load(true);
    } catch {
      errorKey = "errors.microsoft_upload_failed";
    } finally {
...
>
  <div class="flex flex-wrap items-center gap-2 border-b border-border px-3 py-2">
    <button
      ...
      aria-label={t("microsoft.onedrive.up")}
      title={t("microsoft.onedrive.up")}
    >
      <ChevronLeft size={13} aria-hidden="true" />
    </button>
    <nav class="min-w-0 flex-1 truncate text-sm" aria-label={t("microsoft.onedrive.breadcrumb")}>
    ...
    <!-- Server-side, this folder and its subfolders. A box that filtered the array below it
         would be a worse answer than none: the array is one capped page. -->
    <div class="relative shrink-0">
      ...
      <input
        ...
        placeholder={t("microsoft.onedrive.search_placeholder")}
        aria-label={t("microsoft.onedrive.search_placeholder")}
    ...
    {#if pick && listing?.folder?.id && listing?.folder?.drive_id}
      ...
      <form method="POST" action="?/setOneDriveFolder" use:enhance={picked}>
        <input type="hidden" name="entity_type" value={entityType} />
        <input type="hidden" name="entity_id" value={entityId} />
        <input type="hidden" name="drive_id" value={listing.folder.drive_id} />
        <input type="hidden" name="item_id" value={listing.folder.id} />
        <button
          type="submit"
        ...
        >
          ...
          {t("microsoft.onedrive.choose_this_folder")}
        </button>
      </form>
    {/if}
    {#if canWrite && !searching}
      ...
      >
        ...
        {t("microsoft.onedrive.new_folder")}
      </button>
      {#if !pick}
        ...
        >
          ...
          {uploading ? t("microsoft.onedrive.uploading") : t("microsoft.onedrive.upload")}
        </button>
    ...
    <button
      ...
      onclick={() => load(true)}
      aria-label={t("microsoft.onedrive.refresh")}
    >
  ...
  {#if canWrite && creatingFolder && !searching}
    ...
    >
      ...
      <input
        ...
        type="text"
        maxlength="255"
        placeholder={t("microsoft.onedrive.folder_name")}
        aria-label={t("microsoft.onedrive.folder_name")}
      ...
      >
        {savingFolder ? t("common.saving") : t("microsoft.onedrive.create_folder")}
      </button>
      <button
        type="button"
  ...
  {:else if listing}
    {#if listing.query}
      <!-- A list that is silently a subset of a folder tells the same lie the cap does, so
           the header states the term, the count and the subtree it searched. -->
    ...
    {#if listing.truncated}
      <p class="border-b border-border px-3 py-1.5 text-xs text-text-muted">
        {t("microsoft.onedrive.truncated")}
      </p>
    {/if}
    {#if listing.items.length === 0}
      {#if !listing.query}
        <p class="px-3 py-4 text-sm text-text-muted">{t("microsoft.onedrive.empty_folder")}</p>
      {/if}
    {:else}
      <ul class="max-h-72 divide-y divide-border overflow-y-auto">
        {#each listing.items as item (item.id)}
          {@const kind = oneDriveKind(item.mime_type, item.is_folder)}
          ...
          <li class="flex items-center gap-2 px-3 py-2">
            ...
            {:else if canWrite && !pick}
              <!-- The name *is* the link action: same form, same action, same code path as
                   the 🔗 button further along the row, associated by `form=` so the row
                   keeps its layout. ↗ at the end is the only thing that opens OneDrive. -->
              <button
                type="submit"
                form={`onedrive-link-${item.id}`}
                class="group flex min-w-0 flex-1 cursor-pointer items-center gap-1 text-left text-sm text-text hover:underline"
                aria-label={t("microsoft.onedrive.link_file")}
                title={t("microsoft.onedrive.link_file")}
              >
            ...
            {:else if item.web_view_link}
              <!-- Nothing to link here (pick mode, or a caller who cannot write), so the name
                   opens it in OneDrive. A control that always refuses is worse than none. -->
              <a
            ...
            {#if pick}
              {#if item.is_folder}
                ...
                <form method="POST" action="?/setOneDriveFolder" use:enhance={picked}>
                  <input type="hidden" name="entity_type" value={entityType} />
                  <input type="hidden" name="entity_id" value={entityId} />
                  <input type="hidden" name="drive_id" value={item.drive_id} />
                  <input type="hidden" name="item_id" value={item.id} />
                  <button
                    type="submit"
                    class="rounded px-2 py-1 text-xs font-medium text-brand hover:underline"
                  >
                    {t("microsoft.onedrive.choose_folder")}
                  </button>
                </form>
              {/if}
            {:else if canWrite}
              ...
              <form
                id={`onedrive-link-${item.id}`}
                method="POST"
                action="?/linkOneDriveFile"
                use:enhance
              >
                <input type="hidden" name="entity_type" value={entityType} />
                <input type="hidden" name="entity_id" value={entityId} />
            ...
            {/if}
            {#if canWrite && !pick}
              <!-- The same ⋯ item the panel's link list carries. Destructive, so it lives in
                   the overflow menu and confirms — never a naked icon in the row. -->
              <ActionsMenu
                compact
                items={[
                  {
...
<Modal bind:open={confirmTrash} title={t("microsoft.onedrive.trash_title")}>
