 ...
 *  reported only when the org has a provider that can actually transcribe (#246) **and** at
 *  least one feature that dictates (#382). Anthropic has no speech endpoint and is the
 *  default, so without the first half an Anthropic org would be drawn a microphone that 409s
 *  on the first click; without the second, a microphone with nothing to be for. */
