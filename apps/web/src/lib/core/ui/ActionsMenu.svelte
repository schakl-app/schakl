...
<script lang="ts">
  ...
  const triggerClass = $derived(
    compact
      ? "rounded p-1 text-neutral-400 hover:text-neutral-700"
      : "rounded-lg border border-neutral-300 p-2 text-neutral-500 hover:border-brand hover:text-brand",
  );
...
<div class="relative shrink-0" bind:this={root}>
  ...
  {#if open}
    <div
      role="menu"
      class="absolute z-30 mt-1 w-48 rounded-xl border border-neutral-200 bg-white py-1 shadow-lg
      ...
      {#each items as item (item.label)}
        ...
        {#if item.href && !item.disabled}
          <a
            ...
            href={item.href}
            onclick={() => (open = false)}
            class="flex items-center gap-2 px-4 py-2 text-sm hover:bg-neutral-50
              {item.danger ? 'text-red-600 hover:bg-red-50' : 'text-neutral-700'}"
          >
            {#if Icon}<Icon size={15} class={item.danger ? "" : "text-neutral-400"} />{/if}
            {item.label}
          </a>
        {:else}
          <button
            ...
            onclick={() => select(item)}
            class="flex w-full items-center gap-2 px-4 py-2 text-left text-sm hover:bg-neutral-50 disabled:cursor-not-allowed disabled:opacity-50
              {item.danger ? 'text-red-600 hover:bg-red-50' : 'text-neutral-700'}"
          >
            {#if Icon}<Icon size={15} class={item.danger ? "" : "text-neutral-400"} />{/if}
            {item.label}
