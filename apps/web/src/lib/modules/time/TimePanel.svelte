<script lang="ts">
  ...
  const truncated = $derived(totalEntries > recent.length);
...
{#if truncated}
  <!-- It truncated, so it says so — silent truncation reads as "that's all of them". -->
  <p class="mt-1 text-xs text-text-muted">
    {t("time.panel.truncated", { shown: recent.length, total: totalEntries })}
  </p>
{/if}
