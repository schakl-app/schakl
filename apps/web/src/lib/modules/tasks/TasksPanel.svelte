  ...
  <ul class="divide-y divide-neutral-100">
    {#each tasks as task (task.id)}
      ...
      <li class="flex items-center gap-2 py-2">
        <a href={`/tasks/${task.id}`} class="min-w-0 flex-1 truncate text-sm font-medium text-neutral-900 hover:text-brand">
          {task.title}
        </a>
        ...
        {#if (task.checklist_total ?? 0) > 0}
          <span class="text-[11px] tabular-nums text-neutral-400">☑ {task.checklist_done}/{task.checklist_total}</span>
        {/if}
        {#if task.status === "in_progress"}
          <span class="rounded-full bg-brand/10 px-2 py-0.5 text-[11px] font-medium text-brand">{t("tasks.status.in_progress")}</span>
        {/if}
        {#if task.due_date}
          <span class="text-xs tabular-nums {overdue ? 'font-semibold text-red-600' : 'text-neutral-500'}">
...
{/if}
<a href={`/tasks?company_id=${companyId}`} class="mt-3 inline-block text-xs text-brand hover:underline">
