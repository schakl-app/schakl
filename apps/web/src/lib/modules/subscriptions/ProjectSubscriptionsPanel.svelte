<script lang="ts">
  ...
  const total = $derived(
    ((data as Record<string, unknown>)?.total as number | undefined) ?? subscriptions.length,
  );
  /** The register, narrowed to this project's client — the hand-over carries the filter. */
  const companyId = $derived(
    ((data as Record<string, unknown>)?.companyId as string | undefined) ?? "",
  );
</script>
    ...
    {#snippet children(shown)}
      <ul class="divide-y divide-border">
        {#each shown as sub (sub.id)}
          {@const included =
            sub.usage?.included_hours != null ? Number(sub.usage.included_hours) : null}
